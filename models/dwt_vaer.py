...
def dwt_to_time_torch(recon_dwt, lengths, num_features, fixed_cA_len=256, device='cpu'):
    """
    Convert reconstructed DWT features back to variable-length sequences (PyTorch).
    recon_dwt: [batch, num_features * fixed_cA_len]
    Returns list of tensors [seq_len, num_features] on device.
    """
    batch_size = recon_dwt.shape[0]
    recon_time = []
    # reshape per-sample inside loop because lengths (n) vary
    all = recon_dwt.view(batch_size, num_features, fixed_cA_len)  # [B, num_features, fixed_cA_len]
    for i in range(batch_size):
        seq_len = int(lengths[i].item()) if isinstance(lengths[i], torch.Tensor) else int(lengths[i])
        seq_dwt = all[i]  # [num_features, fixed_cA_len]
        # Convert to numpy for DWT reconstruction
        seq_dwt_np = seq_dwt.detach().cpu().numpy()
        # Reconstruct time domain signal using stored meta
        # This will be handled in the main loop where we have access to meta
...
def remove_nan_padding(sample):
    """Remove trailing rows that are all-NaN from a 2D array sample (T_padded, F)."""
    if sample.ndim != 2:
        raise ValueError("sample must be 2D (T, F)")
...
def resample_coeffs(cA, fixed_len):
    """Resample 1D coefficient array cA to fixed_len points using linear interpolation."""
    if len(cA) == fixed_len:
        return cA
...
def sample_to_dwt_approx_only(sample, wavelet='db4', level=4):
    """Extract DWT approximation coefficients for each feature."""
...
def sequences_to_dwt(sequences, fixed_cA_len=256, wavelet='db4', level=4):
    """Convert list of numpy sequences (variable length) to DWT features (resampled cA)"""
    dwt_features = []
    all_metas = []
    if len(sequences) == 0:
    ...
    for seq in sequences:
        ...
        # Resample each cA to fixed length
...
def unresample_and_reconstruct_single(feat_vec, meta, fixed_cA_len):
    """Inverse process: unresample coefficients to original length and inverse DWT."""
    F = meta['num_features']
    recon_features = []
    idx = 0
    ...
    for f in range(F):
        ...
        # Extract resampled coefficients
        cA_resampled = feat_vec[idx: idx + fixed_cA_len]
        idx += fixed_cA_len

        # Interpolate back to original coefficient length
        ...
        # Reconstruct using cA and zeroed detail coeffs
        cDs = [np.zeros(l) for l in detail_lens]
        coeffs = [cA_original_scale] + cDs
        rec = pywt.waverec(coeffs, wavelet)

        # Match original time length
...
def dwt_to_time(recon_dwt, all_metas, num_features, fixed_cA_len=256):
    """Convert reconstructed DWT features (tensor or numpy) back to variable-length numpy sequences"""
...
class DWTMLPVAE(nn.Module):
    def __init__(self, input_dim, latent_dim=64, hidden_dims=[1024, 512, 256]):
        ...
        # Probabilistic regressor for TLX prediction (VAER requirement)
    ...
    def decode(self, z):
        ...
        # Apply decoder layers with residual connections where possible
    ...
    def forward(self, x):
        ...
        # Regressor path: predict from encoder features h (not z!)
        y_mu = self.fc_y_mu(h)
...
class DWTConvVAE(nn.Module):
    def __init__(self, num_features, fixed_cA_len, latent_dim=64):
        ...
        # Encoder: Conv1D along coefficient axis
        # Input: [batch, num_features, fixed_cA_len]
        self.encoder = nn.Sequential(
            nn.Conv1d(num_features, 64, kernel_size=7, stride=2, padding=3),  # -> [batch, 64, cA_len/2]
            nn.BatchNorm1d(64),
            nn.LeakyReLU(0.01),
            
            nn.Conv1d(64, 128, kernel_size=5, stride=2, padding=2),  # -> [batch, 128, cA_len/4]
            nn.BatchNorm1d(128),
            nn.LeakyReLU(0.01),
            
            nn.Conv1d(128, 256, kernel_size=3, stride=2, padding=1),  # -> [batch, 256, cA_len/8]
            nn.BatchNorm1d(256),
            nn.LeakyReLU(0.01),
        )
        ...
        # FIXED: Regressor path branches from encoder features (conv output)
        # This allows sharing of convolutional layers as per paper
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(256, 128, kernel_size=3, stride=2, padding=1, output_padding=1),  # -> [batch, 128, cA_len/4]
            nn.BatchNorm1d(128),
            nn.LeakyReLU(0.01),
            
            nn.ConvTranspose1d(128, 64, kernel_size=5, stride=2, padding=2, output_padding=1),  # -> [batch, 64, cA_len/2]
            nn.BatchNorm1d(64),
            nn.LeakyReLU(0.01),
            
            nn.ConvTranspose1d(64, num_features, kernel_size=7, stride=2, padding=3, output_padding=1),  # -> [batch, num_features, cA_len]
        )
    
    def encode(self, x):
        # x: [batch, input_dim] -> [batch, num_features, fixed_cA_len]
        x = x.view(-1, self.num_features, self.fixed_cA_len)
        h = self.encoder(x)
        h_flat = h.view(h.size(0), -1)  # [batch, conv_output_size]
        
        mu = self.fc_mu(h_flat)
        logvar = self.fc_logvar(h_flat)
        return mu, logvar, h_flat  # Return h_flat for regressor
    ...
    def forward(self, x):
        ...
        # Regressor path: predict from encoder features h_flat (not z!)
...
# -------------------------
# LSTM VAE for DWT coefficients with regression
# -------------------------
...
class DWTLSTMVAE(nn.Module):
    def __init__(self, num_features, fixed_cA_len, latent_dim=64, hidden_size=256, num_layers=2):
        ...
        # Learnable direction vector for conditional prior
...
class DWTVAERTrainer(nn.Module):
    def __init__(self, fixed_cA_len=256, latent_dim=64, hidden_dims=[1024,512,256], lr=1e-3, batch_size=32, epochs=50, wavelet='db4', level=4, device='cuda', use_time_loss=False, architecture='mlp', annealing=None, max_weight=0.5, num_cycles=10):
        super().__init__()
        ...
        self.level = level
        self.use_time_loss = use_time_loss  # Flag to enable/disable time domain loss
    ...
    def get_annealing_weight(self, epoch):
        """Calculate annealing weight for KL and regression losses.
    ...
    def fit(self, padded_data, tlx_targets=None):
        ...
        # 1) Normalize each sequence individually (per-sample zero mean, unit std) -> store stats
        sequences_norm = []
        seq_means = []
        seq_stds = []
        for seq in sequences:
        ...
        # 2) Normalize TLX targets for stable conditional prior
        tlx_normalized = None
        ...
        # 3) Compute DWT features from normalized sequences
        ...
        if self.architecture == 'conv':
            ...
            print(f"Using Conv1D VAER with {num_features} features and {self.fixed_cA_len} coefficients")
        elif self.architecture == 'lstm':
            self.model = DWTLSTMVAE(num_features, self.fixed_cA_len, self.latent_dim).to(self.device)
            print(f"Using LSTM VAER with {num_features} features and {self.fixed_cA_len} coefficients")
        else:
            # Use MLP architecture as default
            self.model = DWTMLPVAE(input_dim, self.latent_dim, self.hidden_dims).to(self.device)
            print(f"Using MLP VAER with input_dim={input_dim}")
        ...
        if tlx_normalized is not None:
            ...
            dataset = TensorDataset(dwt_data, idxs, tlx_tensor)
        else:
        ...
        for epoch in range(self.epochs):
            ...
            for batch_data in loader:
                if tlx_normalized is not None:
                    ...
                    batch_tlx = batch_tlx.to(self.device)  # [batch]
                else:
                ...
                # Conditional KL divergence loss (Brain Aging style)
                if tlx_normalized is not None and batch_tlx is not None:
                    # Use conditional prior p(z|y) = N(u^T * y, σ²I)
                    kl_loss = self.model.conditional_kl_divergence(mu, logvar, batch_tlx)
                else:
                    # Standard KL if no TLX targets
                    kl_loss = torch.mean(-0.5 * torch.sum(1 + logvar - mu**2 - logvar.exp(), dim=1))

                # Regression loss (VAER requirement: -log q(c|x) = negative log-likelihood)
                reg_loss = torch.tensor(0.0, device=self.device)
                if tlx_normalized is not None and batch_tlx is not None:
                    # VAER paper uses -log q(c|x), not KL divergence
                    # Negative log-likelihood of Gaussian: 0.5 * (log(2π) + logvar + (x-μ)²/σ²)
                    reg_loss = 0.5 * torch.mean(
                        y_logvar + ((y_mu.squeeze() - batch_tlx.to(self.device)) ** 2) / torch.exp(y_logvar)
                    )

                # Combined loss with annealing
                if self.annealing:
                    # Apply annealing to KL and regression losses
                    loss = recon_loss + beta * kl_loss + beta * reg_loss
                else:
                    # No annealing: full weight on all terms
                    beta_fixed = 1.0
                    lambda_reg = 1.0 if tlx_normalized is not None else 0.0
                    loss = recon_loss + beta_fixed * kl_loss + lambda_reg * reg_loss
            ...
            if (epoch + 1) % 10 == 0 or epoch == 0:
                if self.annealing:
                    print(f"Epoch {epoch+1:4d}/{self.epochs} | Loss: {avg_loss:.4f} | R: {avg_recon:.4f} | KL: {avg_kl:.4f} | Reg: {avg_reg:.4f} | β={beta:.4f}")
                else:
                    print(f"Epoch {epoch+1:4d}/{self.epochs} | Loss: {avg_loss:.4f} | R: {avg_recon:.4f} | KL: {avg_kl:.4f} | Reg: {avg_reg:.4f}")

        # Save normalization stats (so encode/reconstruct use same normalization)
    ...
    def encode(self, padded_data):
        sequences = extract_variable_length_sequences(padded_data)
        # normalize sequences same as in fit (per-sample)
        sequences_norm = []
        for seq in sequences:
            seq_np = np.asarray(seq, dtype=np.float32)
            mu = seq_np.mean(axis=0, keepdims=True)
        ...
        self.model.eval()
        with torch.no_grad():
            mu, logvar, _ = self.model.encode(dwt_data.to(self.device))  # Ignore encoder features
        return mu.cpu().numpy()
    ...
    def reconstruct(self, dataset_or_data):
        ...
        sequences = extract_variable_length_sequences(padded_data)
        # normalize each sample same as fit
        sequences_norm = []
        seq_means = []
        seq_stds = []
        for seq in sequences:
        ...
        self.model.eval()
        with torch.no_grad():
            recon_dwt, _, _, _, _ = self.model(dwt_data.to(self.device))
        # reconstruct normalized time-domain sequences

...
class DWTVAETrainer(nn.Module):
    ...
    def get_annealing_weight(self, epoch):
        """Calculate annealing weight for KL loss.
    ...
    def fit(self, padded_data):
        ...
        # 2) Compute DWT features from normalized sequences
        ...
        # Initialize model - choose between Conv1D and MLP architecture
        if self.architecture == 'conv':
            self.model = DWTConvVAE(num_features, self.fixed_cA_len, self.latent_dim).to(self.device)
            print(f"Using Conv1D architecture with {num_features} features and {self.fixed_cA_len} coefficients")
        elif self.architecture == 'lstm':
            self.model = DWTLSTMVAE(num_features, self.fixed_cA_len, self.latent_dim).to(self.device)
            print(f"Using LSTM architecture with {num_features} features and {self.fixed_cA_len} coefficients")
        else:  # default to mlp
            self.model = DWTMLPVAE(input_dim, self.latent_dim, self.hidden_dims).to(self.device)
            print(f"Using MLP architecture with input_dim={input_dim}")
        
        optimizer = torch.optim.Adam(self.model.parameters(), lr=self.lr)
        
        # Prepare DataLoader with indices so we can map back to original sequences/metas even if shuffling
        ...
        for epoch in range(self.epochs):
            ...
            for batch_dwt, batch_idxs in loader:
                ...
                # Forward pass
                recon_dwt, mu, logvar = self.model(batch_dwt)  # recon_dwt in normalized-DWT space

                # DWT-domain loss (coefficient domain) - PRIMARY LOSS
                coeff_loss = F.mse_loss(recon_dwt, batch_dwt, reduction='mean')

                # KL divergence (per batch mean)
                kld_loss = torch.mean(-0.5 * torch.sum(1 + logvar - mu**2 - logvar.exp(), dim=1))

                # Optional time domain loss (disabled by default for better coefficient learning)
                time_loss = 0.0
                if self.use_time_loss:
                    batch_metas = [all_metas[int(idx.item())] for idx in batch_idxs]
                    recon_time_list = dwt_to_time(recon_dwt, batch_metas, num_features, self.fixed_cA_len)
                    for i, idx in enumerate(batch_idxs):
                        idx_int = int(idx.item())
                        target_np = sequences_norm[idx_int]
                        target = torch.FloatTensor(target_np).to(self.device)
                        recon = torch.FloatTensor(recon_time_list[i]).to(self.device)
                        min_len = min(recon.shape[0], target.shape[0])
                        time_loss += F.mse_loss(recon[:min_len, :], target[:min_len, :], reduction='mean')
                    time_loss = time_loss / batch_dwt.shape[0]

                # Combined loss with annealing
                if self.annealing:
                    # Apply annealing to KL loss
                    loss = coeff_loss + time_loss + beta * kld_loss
                else:
                    # No annealing: use fixed small weight for KL
                    loss = coeff_loss + time_loss + 1e-3 * kld_loss
            ...
            if (epoch + 1) % 10 == 0 or epoch == 0:
                if self.annealing:
                    print(f"Epoch {epoch+1:4d}/{self.epochs} | Loss: {avg_loss:.4f} | Recon: {avg_recon:.4f} | KL: {avg_kl:.4f} | β={beta:.4f}")
                else:
                    print(f"Epoch {epoch+1:4d}/{self.epochs} | Loss: {avg_loss:.4f} | Recon: {avg_recon:.4f} | KL: {avg_kl:.4f}")

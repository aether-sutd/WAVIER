...
def run_wavelet_ablation_experiments():
    ...
    for wavelet in wavelets:
        for annealing in annealing_methods:
            ...
            print(f"\nTraining: wavelet={wavelet}, annealing={anneal_str}")
            
            # Update parameters
            params = {**base_parameters, 'wavelet': wavelet, 'annealing': annealing}
            ...
            # Save model
            fname = f"dwt_avaer_{wavelet}_{anneal_str}.pt"
    ...
    summary_df = pd.merge(
        train_df, 
        val_df, 
        on=['Wavelet', 'Annealing', 'Model_File']
    )
...
if __name__ == "__main__":
    ...
    print("\n" + "="*80)
    print(" "*15 + "WAVELET TYPE ABLATION STUDY")
    print("="*80)

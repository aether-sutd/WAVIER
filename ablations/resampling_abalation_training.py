...
def run_wavelet_ablation_experiments():
    """Run ablation experiments with different resampling coefficient sizes and annealing methods."""
    ...
    base_parameters = {
        ...
        "wavelet": "sym16",
        "level": 3,
        "mode": "dwt",  # Fixed mode for resampling ablation
        "use_conv": True,
        "device": "cuda",
        "num_cycles": 3
    }
    ...
    coefficient_sizes = [128, 256, 512, 1024, 2048]
    ...
    for coeff_size in coefficient_sizes:
        for annealing in annealing_methods:
            anneal_str = "none" if annealing is None else annealing
            
            print(f"\nTraining: fixed_cA_len={coeff_size}, annealing={anneal_str}")
            
            # Update parameters
            params = {**base_parameters, 'fixed_cA_len': coeff_size, 'annealing': annealing}
            ...
            # Save model
            fname = f"dwt_avaer_coeff{coeff_size}_{anneal_str}.pt"
    ...
    summary_df = pd.merge(
        train_df, 
        val_df, 
        on=['Coeff_Size', 'Annealing', 'Model_File']
    )
...
if __name__ == "__main__":
    ...
    print("\n" + "="*80)
    print(" "*15 + "RESAMPLING COEFFICIENT SIZE ABLATION STUDY")
    print("="*80)
